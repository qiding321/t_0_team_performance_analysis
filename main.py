...
def main():
    ...
    for num, (stock, month) in enumerate(stock_month_list):
        date_last_month = get_date_last_month(month, stock)
        stock_list.append(stock)
        date_list.append(date_last_month)
        this_month_list.append(month)
    ...
    data_new = pd.DataFrame(intraday_characteristics_list, index=list(zip(stock_list, date_list, this_month_list))).reset_index()
    data_new['coid'] = data_new['index'].apply(lambda x: x[0])
    data_new['month_last'] = data_new['index'].apply(lambda x: x[1])
    data_new['month'] = data_new['index'].apply(lambda x: x[2])
    data_new = data_new.drop('index', axis=1)

    data_merged = pd.merge(intraday_team_return_data, data_new, on=['coid', 'month'], how='outer')
...
def one_stock_month_func(date_last_month, stock):
    try:
        ...
        signal = '{}, {}, {}, data loading end, data length: {}'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), stock, date_last_month,
...
def get_date_last_month(month, stock):
